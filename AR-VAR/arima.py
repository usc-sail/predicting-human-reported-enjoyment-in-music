...
OUTPUT_BASENAME = "./results/arima"
ATTENTION_LEN = 40
HORIZONS = ["40", "80"]
CV_FOLDS = ["Beg", "End"]

def run_arima(X_train, y_train, X_test, y_test):
...
def main():
    ...
    parser.add_argument('--attention_len', type=int, default=5)
    ...
    ATTENTION_LEN = para.attention_len
    ...
    for cv in CV_FOLDS:
        ...
        model = ARIMA(X, order=(ATTENTION_LEN, 1, 1))
        model_fit = model.fit()

        # GET PREDICTION ERROR FOR EACH HORIZON
        for horizon in HORIZONS:
            h = int(horizon)
            predictions = np.zeros( (test.shape[0] - h, ) )
            for i in range(test.shape[0] - h):
                y_hat = model_fit.predict(start=X.shape[0] + i + h - 1, end=X.shape[0] + i + h - 1, typ='levels')[0]
                predictions[i] = y_hat
            truth = test[h:]
            ...
            if(cv=="Beg" and horizon=="40" and dataset=="happy_enjoyment.csv"):
                predictions_train = np.zeros((train.shape[0] - h,))
                for i in range(train.shape[0] - h):
                    y_hat = model_fit.predict(start= i + h, end=i + h, typ='levels')[0]
                    predictions_train[i] = y_hat
                truth_train = train[h:]
    ...
    dataset_name = dataset.replace(".csv", "") + "_al_" + str(ATTENTION_LEN)
    ...
    with open(file_out, 'w') as f:
        for horizon in HORIZONS:
            ...
            for set in ["train", "test"]:
                if ( (horizon == "40" and dataset == "happy_enjoyment.csv") or set=="test"):
